...
Chain associations to get an array of the results.  For example, want to see all the 
comments for all blogs written by 'Sarah Johnson'?

  User.find(2).blogs.comments
  [#<Comment id: 21, user_id: 8, blog_id: 85, rating: 5, ... 

Clean this up by just getting the 'comment' fields of each comment and print in a column.

  User.find(2).blogs.comments.comment.pr
  I agree with the blogger
  I love ActiveRecord.
  ActiveRecord is too confusing.

Note that chaining is accumulative.  If you want more specific data, use the 'wi' or 'wo'
commands.
...
act as an OR statement on the results.  So the above example could have been written 

  User.find(2).blogs.comments.wi(:user_id=>5, :user_id=>9).comments.pr

with the same results.

To get an AND effect, chain the 'wi' command.
...
this example returns all comments associated with blog id 6 OR user id 5.
